...
'        Dim argA As Decimal() = in_dAry()

'        Dim h As New Dictionary(Of Decimal, Decimal)
...
'        For i As Decimal = 0 To argA.Count - 1

'            Dim tempA As Decimal = argA(i)

'            For j As Decimal = 0 To Math.Min(N, tempA)
'                h(j) += 1
'            Next
...
'        Dim tempCnt As Decimal = 0

'        For Each k As Decimal In h.Keys.OrderByDescending(Function(kk) kk)
'            If k <= h(k) Then
'                result = k
'                Exit For
'            End If
