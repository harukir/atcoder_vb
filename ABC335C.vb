...
'        Dim moveRireki As New List(Of Decimal())
'        moveRireki.Add({1, 0})
...
'            Dim query As String() = in_sAry()

'            If query(0) = "1" Then

'                Dim nowX As Decimal = moveRireki(moveRireki.Count - 1)(0)
'                Dim nowY As Decimal = moveRireki(moveRireki.Count - 1)(1)

'                Select Case query(1)
'                    Case "U"
'                        moveRireki.Add({nowX, nowY + 1})
'                    Case "D"
'                        moveRireki.Add({nowX, nowY - 1})
'                    Case "L"
'                        moveRireki.Add({nowX - 1, nowY})
'                    Case "R"
'                        moveRireki.Add({nowX + 1, nowY})
'                End Select
...
'                Dim moveCnt As Decimal = moveRireki.Count - 1
'                Dim target As Decimal = Decimal.Parse(query(1))

'                If moveCnt < target Then
'                    r.Append((target - moveCnt).ToString).Append(" ").AppendLine("0")
'                Else
'                    Dim tempPos As Decimal() = moveRireki(moveRireki.Count - target)
'                    r.Append(tempPos(0).ToString).Append(" ").AppendLine(tempPos(1).ToString)
'                End If
