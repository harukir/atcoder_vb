...
'            Dim S As Decimal() = in_dAry()

'            Dim startS As Decimal = S(0)
'            Dim goalS As Decimal = S(S.Count - 1)

'            S = S.OrderBy(Function(ss) ss).ToArray

'            Dim cnt As Decimal = 1

'            Dim nowSize As Decimal = startS
'            Dim isSuccess As Boolean = True

'            For j As Decimal = 0 To S.Count - 1

'                If goalS <= nowSize * 2 Then 'ゴールできるなら問答無用で終わり
'                    cnt += 1
'                    Exit For
'                End If

'                If S(j) <= startS Then Continue For

'                If S(j) <= nowSize * 2 Then

'                    If j + 1 <= S.Count - 1 AndAlso S(j + 1) <= nowSize * 2 Then '次があって、それも条件満たしてたら次ループ
'                        Continue For
'                    End If
...
'            result.AppendLine(If(isSuccess, cnt.ToString, "-1"))
'        Next
