...
'        Dim Q As Decimal = in_d()

'        'Dim argS As String() = in_sAry()
'        'Dim argD As Decimal() = in_dAry()

'        Dim Sary As List(Of String) = splitBlank(S)

'        Dim highOrLow As Decimal = 0 '-1ならLow、1ならHigh、0なら何もしない
'        Dim musiIndex As New HashSet(Of Decimal)
...
'            If argS(0) = "1" Then
'                Sary(Decimal.Parse(argS(1)) - 1) = argS(2)
'                musiIndex.Add(Decimal.Parse(argS(1)) - 1)
'            Else
'                highOrLow = If(argS(0) = "2", -1, 1)
'                musiIndex.Clear()
'            End If
'        Next
...
'        For i As Decimal = 0 To Sary.Count - 1

'            If Not musiIndex.Contains(i) AndAlso highOrLow <> 0 Then
'                If highOrLow = -1 Then
'                    r.Append(Sary(i).ToLower)
'                Else
'                    r.Append(Sary(i).ToUpper)
'                End If
'            Else
'                r.Append(Sary(i))
'            End If
