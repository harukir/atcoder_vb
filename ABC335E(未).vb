...
'            Dim temp As Decimal() = in_dAry()
'            Dim u As Decimal = temp(0) - 1
'            Dim v As Decimal = temp(1) - 1

'            If graph.ContainsKey(u) Then
'                graph(u).Add(v)
'            Else
'                graph.Add(u, New HashSet(Of Decimal)({v}))
'            End If
'            If graph.ContainsKey(v) Then
'                graph(v).Add(u)
'            Else
'                graph.Add(v, New HashSet(Of Decimal)({u}))
'            End If
'        Next
...
'    Sub search(graph As Dictionary(Of Decimal, HashSet(Of Decimal)), A As Decimal(), N As Decimal)

'        Dim movedP As New HashSet(Of Decimal)
'        Dim pointed As New HashSet(Of Decimal)
'        movedP.Add(1)
'        pointed.Add(A(0))

'        dfs(0, graph, A, N, movedP, 1, pointed)

'    End Sub


'    Sub dfs(p As Decimal, graph As Dictionary(Of Decimal, HashSet(Of Decimal)), A As Decimal(), N As Decimal, movedP As HashSet(Of Decimal), tempPoint As Decimal, pointed As HashSet(Of Decimal))

'        For Each pp As Decimal In graph(p)


'            If movedP.Contains(pp) Then Continue For
'            If A(p) > A(pp) Then Continue For

'            Dim temptempPoint As Decimal = tempPoint
'            If Not pointed.Contains(A(pp)) Then temptempPoint += 1


'            If pp = N - 1 Then
'                maxPoint = Math.Max(maxPoint, temptempPoint + 1)
'                Return
'            End If

'            Dim nextMovedP As New HashSet(Of Decimal)(movedP)
'            nextMovedP.Add(pp)
'            Dim nextPointed As New HashSet(Of Decimal)(pointed)
'            nextPointed.Add(A(pp))

'            dfs(pp, graph, A, N, nextMovedP, temptempPoint, nextPointed)
'        Next
